...
public class SqlHolder {
    ...
    public static final String NEW_RETENTION_ANA
            ...
            "  ,deviceType                                                                                           " +
            "                                                                                       " +
            "  ,sum(if( date_format(from_unixtime(nvl(registerTime,firstAccessTime)/1000),'yyyy-MM-dd') = date_format" +
            "( timestampadd(day, -1, CURRENT_TIMESTAMP ), 'yyyy-MM-dd' ) ,1,0) ) as ret_1day        " +
            "  ,sum(if( date_format(from_unixtime(nvl(registerTime,firstAccessTime)/1000),'yyyy-MM-dd') = date_format" +
            "( timestampadd(day, -2, CURRENT_TIMESTAMP ), 'yyyy-MM-dd' ) ,1,0) ) as ret_2day        " +
            "  ,sum(if( date_format(from_unixtime(nvl(registerTime,firstAccessTime)/1000),'yyyy-MM-dd') = date_format" +
            "( timestampadd(day, -3, CURRENT_TIMESTAMP ), 'yyyy-MM-dd' ) ,1,0) ) as ret_3day        " +
            "  ,sum(if( date_format(from_unixtime(nvl(registerTime,firstAccessTime)/1000),'yyyy-MM-dd') = date_format" +
            "( timestampadd(day, -4, CURRENT_TIMESTAMP ), 'yyyy-MM-dd' ) ,1,0) ) as ret_4day        " +
            "  ,sum(if( date_format(from_unixtime(nvl(registerTime,firstAccessTime)/1000),'yyyy-MM-dd') = date_format" +
            "( timestampadd(day, -5, CURRENT_TIMESTAMP ), 'yyyy-MM-dd' ) ,1,0) ) as ret_5day        " +
            "  ,sum(if( date_format(from_unixtime(nvl(registerTime,firstAccessTime)/1000),'yyyy-MM-dd') = date_format" +
            "( timestampadd(day, -6, CURRENT_TIMESTAMP ), 'yyyy-MM-dd' ) ,1,0) ) as ret_6day        " +
            "  ,sum(if( date_format(from_unixtime(nvl(registerTime,firstAccessTime)/1000),'yyyy-MM-dd') = date_format" +
            "( timestampadd(day, -7, CURRENT_TIMESTAMP ), 'yyyy-MM-dd' ) ,1,0) ) as ret_7day        " +
            "from traffic                                                                                            " +
            "                                                                                       " +
