...
public class GuidGenerateFunction extends KeyedProcessFunction<String, EventBean, EventBean> {
    ...
    public void open(Configuration parameters) throws Exception {
        ...
        conn = DriverManager.getConnection("jdbc:mysql://doitedu:3306/rtmk?useUnicode=true&characterEncoding=UTF-8" +
    ...
    public void processElement(EventBean bean, KeyedProcessFunction<String, EventBean, EventBean>.Context ctx,
        ...
        if (StringUtils.isNotBlank(bean.getAccount())) {
            ...
            if (accountId != null) {
                ...
                GuidUtils.deviceAccountBindWeightHandle(bean.getAccount(), bean.getDeviceid(), deviceAccountBindTable
                        , accountId.f0, accountId.f1);


            } else {

                // 从mysql中查询该账号对应的userId和registerTime
                Tuple2<Long, Long> userIdAndRegisterTime = GuidUtils.getUserIdFromMysql(preparedStatement,
                        bean.getAccount());
                ...
                // 去hbase中，更新本账号的设备绑定权重（本账号增加，其他账号衰减）
                GuidUtils.deviceAccountBindWeightHandle(bean.getAccount(), bean.getDeviceid(), deviceAccountBindTable
                        , userIdAndRegisterTime.f0, userIdAndRegisterTime.f1);
        ...
        if (StringUtils.isBlank(bean.getAccount())) {
            ...
            // 如果在缓存中没有找到  设备对应到绑定账号信息，
                ...
                DeviceAccountBindInfo bindAccount = GuidUtils.getMaxBindWeigtAccountInfo(deviceAccountBindTable,
                        bean.getDeviceid());
                ...
                if (bindAccount != null) {
                    ...
                    // 并将查询到的结果信息，放入缓存
                    deviceAccountIdState.put(bean.getDeviceid(), Tuple3.of(bindAccount.getAccount(),
                            bindAccount.getUserId(), bindAccount.getRegisterTime()));
        ...
        if (StringUtils.isBlank(bean.getAccount()) && !flag) {
            ...
            // 如果缓存中没有找到，则去hbase查找临时guid
                ...
                Tuple2<Long, Long> deviceTmpIdFromHbase = GuidUtils.getDeviceTmpIdFromHbase(deviceTmpGuidTable,
                        bean.getDeviceid());
