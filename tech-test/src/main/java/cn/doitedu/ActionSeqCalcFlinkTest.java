    ...
    public static void main(String[] args) throws Exception {
        ...
        SingleOutputStreamOperator<String> res = eventDs.process(new ProcessFunction<Event, String>() {
            ...
            public void open(Configuration parameters) throws Exception {
                String scriptText = FileUtils.readFileToString(new File("tech-test/src/main/java/cn/doitedu/groovy" +
                        "/ActionRuleCalc.groovy"));
                ...
                String ruleParamStr = "{\"ruleId\":\"r01\",\"conditionId\":\"c01\",\"eventSeq\":[\"A\",\"E\",\"C\"]," +
            ...
            @Override
            public void processElement(Event event, ProcessFunction<Event, String>.Context ctx,
                                       Collector<String> out) throws Exception {
                boolean res = calculator.calc(event.getUserId() + "", event.getEventId());
                if (res)
                    out.collect(String.format("用户:%s ,规则:%s ,条件:%s ,最小次数:%d ,已满足 ", event.getUserId(),
                            ruleParam.getString("ruleId"), ruleParam.getString("conditionId"), ruleParam.getInteger(
                                    "minCount")));
            }
        });
